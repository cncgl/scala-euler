...
/**
  ...
  */
object Problem0011 extends App {
  ...
  def maxFour(data: Array[Array[Int]]): Int = {
    var maxVal = 0
    for( i<- 0 until 20; j<- 0 until 20) {
      if(j <= 16) {
        val v = data(i)(j) * data(i)(j+1) * data(i)(j+2) * data(i)(j+3)
        if(v > maxVal) maxVal = v
      }
      if(j >= 3 && i <= 16) {
        val v = data(i+3)(j-3) * data(i+2)(j-2) * data(i+1)(j-1) * data(i)(j)
        if(v > maxVal) maxVal = v
      }
      if(i <= 16) {
        val v = data(i+3)(j) * data(i+2)(j) * data(i+1)(j) * data(i)(j)
        if(v > maxVal) maxVal = v
      }
      if(j <= 16 && i <= 16) {
        val v = data(i)(j) * data(i+1)(j+1) * data(i+2)(j+2) * data(i+3)(j+3)
        if(v > maxVal) maxVal = v
      }
    }
    maxVal
